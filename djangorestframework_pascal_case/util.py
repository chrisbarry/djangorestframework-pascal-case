...
pascalize_re = re.compile(r"[a-z0-9]?_[a-z0-9]")


def underscore_to_pascal(match):
    group = match.group()
    if len(group) == 3:
...
def pascalize(data, **options):
    """Convert underscore_case keys to PascalCase."""
    ...
    if isinstance(data, dict):
        ...
        for key, value in data.items():
            ...
            if isinstance(key, str):
                if "_" in key:
                    new_key = re.sub(pascalize_re, underscore_to_pascal, key)
                else:
                    new_key = key
            ...
            if key not in ignore_fields and new_key not in ignore_fields:
                result = pascalize(value, **options)
            else:
                result = value
    ...
    if is_iterable(data) and not isinstance(data, str):
        return [pascalize(item, **options) for item in data]
    return data


# Keep camelize as an alias for backwards compatibility
camelize = pascalize
